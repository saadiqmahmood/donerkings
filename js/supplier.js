...
document.querySelectorAll('.saveBtn').forEach(function(saveBtn) {
    saveBtn.addEventListener('click', function() {
        ...
        .then(text => {
            speakText('Supplier details updated successfully.');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (urlParams.has('addSuccess')) {
        speakText('New supplier added successfully.');
        showNotification("New supplier added successfully", 5000);
...
document.getElementById('deleteSelectedBtn').addEventListener('click', function () {
    var selectedIds = Array.from(document.querySelectorAll('.supplierCheckbox:checked')).map(checkbox => checkbox.value);

    if (selectedIds.length === 0) {
        speakText('Please select at least one supplier to delete.');
        alert('Please select at least one supplier to delete.');
        return;
    }

    speakText('Are you sure you want to delete the selected suppliers?');
    if (confirm('Are you sure you want to delete the selected suppliers?')) {
        var formData = new URLSearchParams();
        selectedIds.forEach(id => formData.append('supplier_id[]', id)); // Match the PHP $_POST key
        ...
        fetch('supplier.php', {
            ...
            .then(text => {
                speakText('Selected suppliers deleted successfully.');
                sessionStorage.setItem('deleteSuccess', 'Selected suppliers deleted successfully.');
...
document.querySelectorAll('.productCheckbox, .editIcon, .saveBtn, .cancelBtn, .optionsIcon').forEach(function(icon) {
